...
- (id)initWithDictionary:(NSDictionary *)dictionary {
	...
	if ((self = [super init])) {
		for (NSString *key in [JastorRuntimeHelper propertyNames:[self class]]) {
            ...
            NSString *typeName = [JastorRuntimeHelper propertyTypeForPropertyName:key ofObject:self];
			
//            NSLog(@"typeName=@%@",typeName);
			// handle dictionary
			if ([value isKindOfClass:nsDictionaryClass]&&(![typeName containsString:@"NSMutableArray"]&&![typeName containsString:@"NSArray"])) {
			...
			// handle array
				...
				for (id child in value) {
                    if ([[child class] isSubclassOfClass:nsDictionaryClass]) {
                        ...
                        //NSArray * array = [[NSArray alloc]initWithArray:(NSArray *)value];
			...
			// handle all others
            else if([typeName containsString:@"NSMutableArray"]||[typeName containsString:@"NSArray"]){
                NSLog(@"other array");
                
                NSMutableArray *childObjects = [NSMutableArray arrayWithCapacity:1];
//                for (id child in value)
                {
                    if ([[value class] isSubclassOfClass:nsDictionaryClass]) {
                        Class arrayItemType = [[self class] performSelector:NSSelectorFromString([NSString stringWithFormat:@"%@_class", key])];
                        if ([arrayItemType isSubclassOfClass:[NSDictionary class]]) {
                            [childObjects addObject:value];
                        } else if ([arrayItemType isSubclassOfClass:[Jastor class]]) {
                            Jastor *childDTO = [[[arrayItemType alloc] initWithDictionary:value] autorelease];
                            [childObjects addObject:childDTO];
                        }
                    } else {
                        [childObjects addObject:value];
		...
		id objectIdValue;
        @try {
            if ([[dictionary allKeys] containsObject:@"idPropertyName"]) {
